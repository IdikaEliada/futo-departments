...
describe("getDepartmentAbbreviation()", () => {
  it("returns the correct code for a known department", () => {
    expect(getDepartmentAbbreviation("Computer Science")).toBe("CSC");
    expect(getDepartmentAbbreviation("Medicine and Surgery (MBBS)")).toBe("MBBS");
  });

  it("returns undefined for an unknown department (no fallback)", () => {
    expect(getDepartmentAbbreviation("Data Science")).toBeUndefined();
    expect(getDepartmentAbbreviation("")).toBeUndefined();
  });
});
...
describe("getFacultyAbbreviation()", () => {
  ...
  it("returns undefined for an unknown faculty (no fallback)", () => {
    expect(getFacultyAbbreviation("School of Unknown")).toBeUndefined();
  });
});

// ─── getDepartmentByAbbreviation ──────────────────────────────────────────────

describe("getDepartmentByAbbreviation()", () => {
  it("finds a department by its code", () => {
    expect(getDepartmentByAbbreviation("CSC")).toBe("Computer Science");
    expect(getDepartmentByAbbreviation("MBBS")).toBe("Medicine and Surgery (MBBS)");
  });
  ...
  it("returns undefined for an unknown code", () => {
    expect(getDepartmentByAbbreviation("XYZ")).toBeUndefined();
    expect(getDepartmentByAbbreviation("")).toBeUndefined();
  });
});

// ─── getFacultyByAbbreviation ─────────────────────────────────────────────────

describe("getFacultyByAbbreviation()", () => {
  it("finds a faculty by its code", () => {
    expect(getFacultyByAbbreviation("COM")).toBe("College of Medicine");
    expect(getFacultyByAbbreviation("SICT")).toBe(
      "School of Information & Communication Technology"
    );
  });
  ...
  it("returns undefined for an unknown code", () => {
    expect(getFacultyByAbbreviation("XYZ")).toBeUndefined();
  });
});

// ─── listDepartmentsWithAbbreviations ─────────────────────────────────────────

describe("listDepartmentsWithAbbreviations()", () => {
  it("returns an array with the same length as departments", () => {
    expect(listDepartmentsWithAbbreviations()).toHaveLength(departments.length);
  });

  it("each item has name and abbreviation fields", () => {
    const list = listDepartmentsWithAbbreviations();
    for (const item of list) {
      expect(item).toHaveProperty("name");
      expect(item).toHaveProperty("abbreviation");
      expect(typeof item.name).toBe("string");
      expect(typeof item.abbreviation).toBe("string");
    }
  });

  it("includes a known department with its correct abbreviation", () => {
    const list = listDepartmentsWithAbbreviations();
    const cs = list.find((d) => d.name === "Computer Science");
    expect(cs).toBeDefined();
    expect(cs?.abbreviation).toBe("CSC");
  });
});

// ─── listFacultiesWithAbbreviations ──────────────────────────────────────────

describe("listFacultiesWithAbbreviations()", () => {
  it("returns an array with the same length as faculties", () => {
    expect(listFacultiesWithAbbreviations()).toHaveLength(faculties.length);
  });

  it("each item has name and abbreviation fields", () => {
    const list = listFacultiesWithAbbreviations();
    for (const item of list) {
      expect(item).toHaveProperty("name");
      expect(item).toHaveProperty("abbreviation");
    }
  ...
  it("includes a known faculty with its correct abbreviation", () => {
    const list = listFacultiesWithAbbreviations();
    const med = list.find((f) => f.name === "College of Medicine");
    expect(med?.abbreviation).toBe("COM");
  });
});